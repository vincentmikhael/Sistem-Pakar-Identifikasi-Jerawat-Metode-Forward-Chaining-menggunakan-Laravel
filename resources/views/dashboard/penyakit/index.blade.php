...
@section('content')
<h1 class="display-6">Penyakit</h1>
<div class="d-flex justify-content-between align-items-center mt-3">
  ...
  <a href="/export-penyakit" class="btn btn-danger">Export PDF</a>
</div>
...
<script>
  ...
  modal.addEventListener('show.bs.modal', event => {
    ...
    modalTitle.textContent = `Tambah Penyakit`
    form.id.value = null

    if(data){ // jika tombol edit ditekan
      data = JSON.parse(data)
      
      modalTitle.textContent = `Edit Penyakit`
      form.nama.value = data.nama
      form.id.value = data.id
      form.penjelasan.value = data.penjelasan
      form.tindakan.value = data.tindakan
    }
  ...
  btn_delete.forEach(button => {
    ...
    .then((willDelete) => {
      if (willDelete) {
        axios.post('/penyakit/'+this.getAttribute('data-id'),{
          _method: 'DELETE'
        }).then(e=>{
          if(e.status == 200){
            location.reload();
